...
public class Jpg2Dcm {
    protected static final int BUFFER_SIZE = 8162;
    protected static final ElementDictionary DICT = ElementDictionary.getStandardElementDictionary();
    protected static final ResourceBundle rb = ResourceBundle.getBundle("org.dcm4che3.tool.jpg2dcm.messages");

    protected static final int[] IUID_TAGS = {
            Tag.StudyInstanceUID,
            Tag.SeriesInstanceUID,
            Tag.SOPInstanceUID
    };

    protected static final int[] TYPE2_TAGS = {
            Tag.ContentDate,
            Tag.ContentTime
    };

    protected boolean noAPPn;
    protected boolean photo;
    protected Attributes staticMetadata = new Attributes();
    protected byte[] buf = new byte[BUFFER_SIZE];

    protected void setNoAPPn(boolean noAPPn) {
        this.noAPPn = noAPPn;
    }

    protected void setPhoto(boolean photo) {
        this.photo = photo;
    }
...
//    }
    ...
    class Jpg2DcmFileVisitor extends SimpleFileVisitor<Path> {
        ...
        protected Path resolveDestFilePath(Path srcFilePath) {
    ...
    protected void convert(Path srcFilePath, Path destFilePath) throws Exception {
    ...
    protected void copyPixelData(SeekableByteChannel channel, long position, DicomOutputStream dos, byte... prefix)
            throws IOException {
    ...
    protected void copy(ByteChannel in, OutputStream out) throws IOException {
    ...
    protected static void supplementMissingUIDs(Attributes metadata) {
        for (int tag : IUID_TAGS)
            if (!metadata.containsValue(tag))
                metadata.setString(tag, VR.UI, UIDUtils.createUID());
    }

    protected static void supplementMissingValue(Attributes metadata, int tag, String value) {
        if (!metadata.containsValue(tag))
            metadata.setString(tag, DICT.vrOf(tag), value);
    }

    protected static void supplementType2Tags(Attributes metadata) {
